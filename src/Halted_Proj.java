...
public class Halted_Proj extends JPanel {
	int sum;
	String[] names=null;
	/**
	 * Create the panel.
	...
	public Halted_Proj() {
		...
		try {
			...
			String str="select count(proj_name) from addproject where status='Halted'";
		...
		try {
			Class.forName("com.mysql.jdbc.Driver");
			Connection cn=(Connection) DriverManager.getConnection("jdbc:mysql://localhost/erp?user=root&password=Sarthak@124");
			String str="select proj_name from addproject where status='Halted'";
			java.sql.PreparedStatement ps4=cn.prepareStatement(str);
			ResultSet rs=ps4.executeQuery();
			while(rs.next())
			{
