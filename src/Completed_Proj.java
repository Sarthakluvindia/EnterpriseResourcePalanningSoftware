...
public class Completed_Proj extends JPanel {

	int sum;
	String[] names=null;
	public Completed_Proj() {
		...
		try {
			...
			String str="select count(proj_name) from addproject where status='Completed'";
		...
		try {
			Class.forName("com.mysql.jdbc.Driver");
			Connection cn=(Connection) DriverManager.getConnection("jdbc:mysql://localhost/erp?user=root&password=Sarthak@124");
			String str="select proj_name from addproject where status='Completed'";
			java.sql.PreparedStatement ps4=cn.prepareStatement(str);
			ResultSet rs=ps4.executeQuery();
			while(rs.next())
			{
				...
				for(int i=0;i<names.length;i++)
				{
					...
					proj[i].addActionListener(new ActionListener() {
						...
						public void actionPerformed(ActionEvent arg0) {
							...
							removeAll();
							setLayout(null);
							add(new CompleteProject(proj_name));
							updateUI();
						}
					});
